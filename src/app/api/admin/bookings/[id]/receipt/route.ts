...
export async function POST(
  ...
  // If booking is still pending_payment, advance it to pending_verification
  const newStatus = booking.status === 'pending_payment' ? 'pending_verification' : booking.status

  const { data: updated, error: updateError } = await supabase
    .from('bookings')
    .update({ payment_receipt_url: uploadData.path, status: newStatus })
    .eq('id', id)
    .select()
    .single()
